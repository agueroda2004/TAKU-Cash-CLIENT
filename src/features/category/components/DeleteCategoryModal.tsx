...
export default function DeleteCategoryModal({
  ...
  const { deleteCategory, deactivateCategory, isDeleting, isDeactivating } =
    useCategory();
  ...
  async function handleHardDelete() {
    ...
    try {
      ...
      notify({ success: true, message: "Categoría eliminada" });
      reset();
      onCancel();
    } catch (err) {
      if (isApiErrorCode(err, ErrorCode.Conflict)) {
        setBlocked(true);
        setSubmitError(
          "Esta categoría tiene transacciones asociadas, no se puede eliminar.",
        );
      } else {
        setSubmitError(
  ...
  async function handleDeactivate() {
    ...
    try {
      ...
      notify({ success: true, message: "Categoría desactivada" });
      reset();
      onCancel();
    } catch (err) {
      setSubmitError(
        err instanceof Error ? err.message : "Error al desactivar la categoría",
      );
    }
  }
      ...
      >
        ...
        <div className="flex flex-col-reverse gap-2 border-t border-zinc-100 px-6 py-4 sm:flex-row sm:justify-end">
          ...
          {blocked ? (
            ...
            >
              {isDeactivating ? "Desactivando..." : "Desactivar categoría"}
            </button>
          ) : (
            <>
              <button
