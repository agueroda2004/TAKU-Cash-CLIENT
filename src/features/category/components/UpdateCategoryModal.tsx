...
import { CircleHelp, EyeOff, Plus, RotateCcw, Trash2, TriangleAlert } from "lucide-react";
...
export default function UpdateCategoryModal({ open, category, onCancel }: Props) {
  ...
  const pendingDeletes = useMemo(
    () => drafts.filter((d): d is ExistingDraft => d.kind === "existing" && d.pendingDelete),
    [drafts],
  );
  ...
  const hasRenamed = drafts.some(
    (d) => d.kind === "existing" && !d.pendingDelete && d.name.trim() !== d.originalName,
  );
  const hasStatusChanged = drafts.some(
    (d) => d.kind === "existing" && !d.pendingDelete && d.isActive !== d.originalIsActive,
  );
  ...
  const dirty =
    nameDirty || colorDirty || iconDirty || isActiveDirty || hasNewSubcategories || hasRenamed || hasStatusChanged || hasPendingDeletes;
  ...
  function markForDelete(id: string) {
    setDrafts((prev) =>
      prev.map((d) =>
        d.kind === "existing" && d.id === id ? { ...d, pendingDelete: true } : d,
      ),
    );
  }
  ...
  function removeNew(clientId: string) {
    setDrafts((prev) => prev.filter((d) => !(d.kind === "new" && d.clientId === clientId)));
  }
  ...
  async function handleSave() {
    ...
    const updatePayload = drafts
      .filter(
        (d): d is ExistingDraft =>
          d.kind === "existing" &&
          !d.pendingDelete &&
          (d.name.trim() !== d.originalName || d.isActive !== d.originalIsActive),
      )
      .map((d) => ({
        id: d.id,
        name: d.name.trim(),
    ...
    const deletePayload = drafts
      .filter((d): d is ExistingDraft => d.kind === "existing" && d.pendingDelete)
      .map((d) => d.id);
    ...
    <Overlay>
      ...
      >
        <div className="shrink-0 border-b border-zinc-100 px-6 py-4">
          <h2 className="text-lg font-bold text-zinc-800">Editar categoría</h2>
        ...
        <div className="space-y-4 overflow-y-auto px-6 py-5">
          ...
          <div>
            <div className="mb-3 flex items-start gap-2 rounded-xl bg-blue-50 px-4 py-3 text-xs text-blue-700">
              ...
              <p>
                Las subcategorías te permiten dividir una categoría para tener un mejor control
                y reportes de gastos e ingresos.
              </p>
            </div>
            ...
            <div className="mb-3 flex items-start gap-2 rounded-xl bg-amber-50 px-4 py-3 text-xs text-amber-700">
              ...
              <p>
                Si desactivas una subcategoría no se mostrará en el sistema, pero los datos
                históricos se conservan. Puedes reactivarla después.
              </p>
            </div>
            ...
            {hasPendingDeletes && (
              <div className="mt-3 space-y-2">
                <div className="flex items-start gap-2 rounded-xl bg-red-50 px-4 py-3 text-xs text-red-700">
                  ...
                  <p>
                    Si una subcategoría tiene transacciones asociadas no se podrá eliminar.
                    En ese caso, puedes desactivarla para ocultarla del sistema.
                  </p>
                </div>
                ...
                {pendingDeletes.map((d) => (
                  ...
                  >
                    <span className="flex-1 line-through">{d.originalName}</span>
                    <button
                      type="button"
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
              <div className="flex-1">
                ...
                <p className="text-xs text-zinc-500">
                  Las categorías inactivas no aparecen en selecciones ni
                  reportes
                </p>
              </div>
              <button
